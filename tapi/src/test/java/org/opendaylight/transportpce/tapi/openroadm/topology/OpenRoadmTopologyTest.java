...
import org.opendaylight.transportpce.tapi.openroadm.topology.storage.OpenRoadmStorage;
import org.opendaylight.transportpce.tapi.openroadm.topology.storage.TAPI;
import org.opendaylight.transportpce.tapi.openroadm.topology.storage.TAPIStorage;
import org.opendaylight.transportpce.tapi.openroadm.topology.terminationpoint.OpenRoadmTerminationPoint;
import org.opendaylight.transportpce.tapi.openroadm.topology.terminationpoint.factory.OwnNodeEdgePointFactory;
...
class OpenRoadmTopologyTest {
    ...
    void copyTopologyToTAPI() throws ExecutionException, InterruptedException {
        ...
        TAPI tapiStorageMock = new TAPIStorage(networkTransactionService);

        OpenRoadmTopology copyFromOpenRoadm =
            new OpenRoadmTopology(
                    tapiContext,
                    tapiStorageMock,
                    new OpenRoadmStorage(networkTransactionService),
                    new OwnNodeEdgePointFactory(new OpenRoadmTerminationPoint(networkTransactionService), tapiContext)
            );
        ...
        assertTrue(copyFromOpenRoadm.copyTopologyToTAPI(topologyUpdateResultBuilder.build()));
    }
