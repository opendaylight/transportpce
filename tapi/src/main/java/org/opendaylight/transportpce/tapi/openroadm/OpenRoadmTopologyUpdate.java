...
public class OpenRoadmTopologyUpdate implements TopologyUpdate {
    ...
    public boolean copyToTAPI(TopologyUpdateResult topologyUpdateResult) {
        ...
        Set<TerminationPointMapping> mappings =
                terminationPointMappingFactory.create(topologyUpdateResult, openRoadmTopo);
        Set<OwnedNodeEdgePointSpectrumCapability> scaps =
                topologyChangesExtractor.spectrumCapabilityPacs(mappings);
    ...
    private void updateSingleNep(OwnedNodeEdgePointSpectrumCapability cap) {
        ...
        OwnedNodeEdgePoint existingNep = existing.orElse(null);
        if (existingNep == null) {
            LOG.warn("Owned node edge point {} not found in TAPI Topology", nepId.toLogString());
            return;
        }

        OwnedNodeEdgePoint updated = withSpectrumCapabilityPac(existingNep, cap);
        ownedNodeEdgePointRepository.update(nepId, updated);
        LOG.info("Updated spectrum capabilities on Owned Node Edge Point '{}'", cap.nepName().toString());
    }
    ...
    private Network readOpenRoadmTopology() {
        ...
        return openRoadmTopologyRepository.read(
