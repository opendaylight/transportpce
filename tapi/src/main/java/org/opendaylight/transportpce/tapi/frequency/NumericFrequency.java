...
public class NumericFrequency implements Numeric {
    ...
    @Override
    public Map<Double, Double> assignedFrequency(Assigned frequency) {
        return this.range(frequency.assignedFrequencies());
    }

    @Override
    public Map<Double, Double> availableFrequency(Assigned frequency) {
        return this.range(frequency.availableFrequencies());
    }

    /**
     * Note: This code including original comment is mostly copied as is from the original
