...
public final class ConnectivityUtils {
    ...
    private Map<ConnectionKey,Connection> createXpdrCepsAndConnectionsDsr(
        ...
        org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.connectivity.rev221121
                .connectivity.context.Connection connectionDsr =
            createTopConnection(spcXpdr1, spcXpdr2, cepMapDsr, TapiConstants.DSR, LayerProtocolName.DSR,
                    getLowerConnectionMap(TapiConstants.DSR), this.topConnXpdrXpdrPhtn);
        populateConnectionInConnectionVsService(serviceName, serviceUuid,
            connectionDsr.getName().entrySet().stream()
                .filter(nm -> nm.getKey().equals(new NameKey("Connection name"))).findAny().orElseThrow()
                .getValue().getValue(),
        ...
        updateXpdrNepPayloadStructure(getAssociatedNetworkPort(spcXpdr1, xpdrNetworkTplist), TapiConstants.I_ODU, rate);
        updateXpdrNepPayloadStructure(getAssociatedNetworkPort(spcXpdr2, xpdrNetworkTplist), TapiConstants.I_ODU, rate);
