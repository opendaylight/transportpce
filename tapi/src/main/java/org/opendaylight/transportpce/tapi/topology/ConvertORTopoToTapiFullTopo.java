...
public class ConvertORTopoToTapiFullTopo {
    ...
    public void convertRdmToRdmLinks(
        ...
        for (var link : rdmTordmLinkList) {
            ...
            LOG.info("CONVERTTOFULL147, cepMap is {}", cepMap);
    ...
    private void addCepToOnepAndNode(Map<Map<String, String>, ConnectionEndPoint> cepMap) {
        ...
        for (Map.Entry<Map<String, String>, ConnectionEndPoint> cepEntry : cepMap.entrySet()) {
            ...
            LOG.info("CONVERTTOFULL165, Node UUID is {}", nepNodeId);
            ...
            LOG.info("CONVERTTOFULL166, TapiNode Keys are {}", tapiNodes
                .entrySet().stream()
                .map(Map.Entry::getKey)
                .collect(Collectors.toList()));
            LOG.info("CONVERTTOFULL172, TapiNode Keys are {}", tapiNodes
                .entrySet().stream()
                .map(nep -> nep.getValue().getName().toString())
                .collect(Collectors.toList()));
//            LOG.info("CONVERTTOFULL170, TapiNode Keys are {}", tapiNodes
//                .entrySet().stream()
//                .filter(theNode -> theNode.getKey().getUuid().toString().equals(nepNodeId))
//                .map(Map.Entry::getValue)
//                .collect(Collectors.toList()));
            if (!listKey.toString().contains(nepNodeId)) {
                LOG.info("ConvertToFullLINE180, ListKey {} of TapiNodes does not contain NodeUuid {}",
                    listKey, nepNodeId);
            }
            ...
            LOG.info("CONVERTTOFULL191, successfully create node with Node {} CepList {} ", newNode.getName(), cepList);
        }
    }
