...
public class ConvertORTopoToTapiFullTopo {
    ...
    public void convertRdmToRdmLinks(
        ...
        for (org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev180226
                .networks.network.Link link : rdmTordmLinkList) {
            if (!linksToNotConvert.contains(link.getLinkId().getValue())) {
                org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev180226
                        .networks.network.Link oppositeLink = rdmTordmLinkList.stream()
    ...
    public void convertRoadmNode(Node roadm, Network openroadmTopo) {
        ...
        Uuid nodeUuid = new Uuid(UUID.nameUUIDFromBytes(
                (String.join("+", roadm.getNodeId().getValue(), TapiStringConstants.PHTNC_MEDIA))
        ...
        Name nameNodeType = new NameBuilder()
            ...
            .setValue(this.ietfNodeType.getName())
            .build();
        // Protocol Layer
        ...
        org.opendaylight.yang.gen.v1.urn.onf.otcc.yang.tapi.topology.rev221121.topology.Node roadmNode =
            createRoadmTapiNode(
                nodeUuid, Map.of(nodeNames.key(), nodeNames, nameNodeType.key(), nameNodeType),
                layerProtocols, oneplist);
        ...
        for (TerminationPoint tp:tpList) {
            ...
            Name nepName = new NameBuilder()
                ...
                .setValue(
                    String.join("+", this.ietfNodeId, nepPhotonicSublayer, tp.getTpId().getValue()))
                .build();
            List<SupportedCepLayerProtocolQualifierInstances> sclpqiList = new ArrayList<>(List.of(
    ...
    public void convertXpdrToRdmLinks(
        ...
        for (org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev180226
                .networks.network.Link link:xpdrRdmLinkList) {
            if (!linksToNotConvert.contains(link.getLinkId().getValue())) {
                org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev180226
                        .networks.network.Link oppositeLink =
                    xpdrRdmLinkList.stream()
