...
public interface TapiLink {
     ...
     * @param linkResolver Topology containing the openroadm link.
     * @return the created TAPI link, or {@code null} if the link type is not recognized
     *         or the link cannot be created
     * @see #createTapiLink(org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev180226.networks.network.Link, Network, Uuid, LinkTerminationPointsFactory)
     */
    Link createTapiLink(
     ...
     * @param transLayerNameList set of transitioned layer protocol names
     * @param tapiTopoUuid UUID of the TAPI topology that will contain the link
     * @return the created TAPI link, or {@code null} if the link type is not recognized
     *         or the link cannot be created
