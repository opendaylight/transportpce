...
public class TapiTopologyImpl implements TapiTopologyService {
    ...
    public TapiTopologyImpl(NetworkTransactionService networkTransactionService, TapiContext tapiContext,
                            DataBroker dataBroker, TopologyUtils topologyUtils) {
    ...
    public ListenableFuture<RpcResult<GetTopologyDetailsOutput>> getTopologyDetails(GetTopologyDetailsInput input) {
        ...
        try {
            ...
            Topology topology = this.topologyUtils.createAbstractedOtnTopology();
            if (TopologyUtils.TPDR_100G.equals(input.getTopologyIdOrName())) {
                topology = this.topologyUtils.createAbstracted100GTpdrTopology(topology);
            }
