...
public class R2RTapiLinkDiscovery {
    ...
    public Map<LinkKey, Link> readLLDP(NodeId nodeId, String nodeVersion, Uuid tapiTopoUuid) {
        ...
        if (nodeVersion.equals(NodeInfo.OpenroadmVersion._121.getName())) {
            ...
            if (!protocolObject.isPresent() || (protocolObject.get().augmentation(Protocols1.class) == null)) {
                LOG.warn("Tapi R2R Link LLDP subtree is missing : isolated openroadm device");
                return new HashMap<>();
            }
            Map<LinkKey, Link> linkMap = new HashMap<>();
            NbrList nbrList = protocolObject.get().augmentation(Protocols1.class).getLldp().getNbrList();
            LOG.info("Tapi R2R Link LLDP subtree is present. Device has {} neighbours", nbrList.getIfName().size());
        ...
        else if (nodeVersion.equals(NodeInfo.OpenroadmVersion._221.getName())) {
            ...
            LOG.error("Tapi R2R Link Unable to read LLDP data for unmanaged openroadm device version");
            return new HashMap<>();
        }
    }
    ...
    public Link createR2RTapiLink(NodeId nodeId, String interfaceName, String remoteSystemName,
        ...
        LOG.info("Tapi R2R Link OMS link created = {}", omsLink.toString());
        return omsLink;
    }
