 ...
 */
public class ConvertTopoORtoTapiNbi {

    private static final Logger LOG = LoggerFactory.getLogger(ConvertTopoORtoTapiNbi.class);
     ...
     */
    public ConvertTopoORtoTapiNbi(Uuid tapiTopoUuid, TapiLink tapiLink) {
    ...
    public void convertRoadmInfrastructure() {
        ...
        Uuid nodeUuid = new Uuid(UUID.nameUUIDFromBytes(TapiConstants.RDM_INFRA
            .getBytes(StandardCharsets.UTF_8)).toString());
        Name nodeName =
            new NameBuilder().setValueName("otsi node name").setValue(TapiConstants.RDM_INFRA).build();
        Name nodeName2 =
            new NameBuilder().setValueName("roadm node name").setValue(TapiConstants.RDM_INFRA).build();
        Name nameNodeType =
