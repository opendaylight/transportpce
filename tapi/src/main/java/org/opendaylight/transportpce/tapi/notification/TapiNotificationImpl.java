...
public class TapiNotificationImpl implements TapiNotificationService {
    ...
    public ListenableFuture<RpcResult<CreateNotificationSubscriptionServiceOutput>>
        ...
        try {
            ...
            if (!createTapiTopic(createTapiTopicNotification(uuid.getValue()))) {
                ...
                return RpcResultBuilder.<CreateNotificationSubscriptionServiceOutput>failed()
                    .withError(RpcError.ErrorType.RPC, "Could not create tapi topic in kafka")
                    .buildFuture();
            }
    ...
    private TapiTopicNotification createTapiTopicNotification(String topic) {
        if (topic == null) {
            LOG.error("Tapi topic is null");
            return null;
        }
        return new TapiTopicNotificationBuilder()
            .setTopic(topic)
            .build();
    }

    private boolean createTapiTopic(TapiTopicNotification topicNotification) {
        // Create tapi topic by sending notification to nbi notifications module which has a listener
        try {
            this.notificationPublishService.putNotification(topicNotification);
            return true;
