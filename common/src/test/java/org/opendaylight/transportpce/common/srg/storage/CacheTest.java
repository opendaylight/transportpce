...
class CacheTest {
    ...
    void readFromStorage() {
        ...
        Storage fakeStore = Mockito.mock(Storage.class);
        Mockito.when(fakeStore.read(new NetworkNodesKey("ROADM-A-SRG1"))).thenReturn(sharedRiskGroupMap);

        Storage cache = new Cache(fakeStore);

        Assert.assertEquals(sharedRiskGroupMap, cache.read("ROADM-A-SRG1"));

        Mockito.verify(fakeStore, Mockito.times(1)).read(new NetworkNodesKey("ROADM-A-SRG1"));
    }
    ...
    void cacheUpdatedOnWrite() {
        ...
        Mockito.verify(fakeStore, Mockito.times(1)).read(new NetworkNodesKey("ROADM-B-SRG1"));
    }
