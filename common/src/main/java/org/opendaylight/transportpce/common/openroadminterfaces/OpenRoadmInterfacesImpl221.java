            ...
            if (ifBuilder.getSupportingCircuitPackName() != null && ifBuilder.getSupportingPort() != null) {
                ...
                while (!devicePortIsUptodated && !timer.isInterrupted()) {
                    ...
                    if (!devicePortIsUptodated) {
                        try {
                            LOG.debug("Interface {} not found on node {}, waiting 1 second before retrying...",
                                    ifBuilder.getName(),
                                    nodeId);
                ...
                if (devicePortIsUptodated) {
                    LOG.info("Confirmed, {} - {} - interface {} updated on port {}",
                            nodeId,
                            ifBuilder.getSupportingCircuitPackName(),
                            ifBuilder.getName(),
                            ifBuilder.getSupportingPort());
                } else {
                    LOG.warn("Unable to confirm, {} - {} - interface {} updated on port {}",
                            nodeId,
                            ifBuilder.getSupportingCircuitPackName(),
                            ifBuilder.getName(),
                            ifBuilder.getSupportingPort());
                ...
                LOG.info("Deleted interface {} on node {}, not waiting for confirmation from node",
                        ifBuilder.getName(),
                        nodeId);
            }
        } catch (InterruptedException | ExecutionException e) {
