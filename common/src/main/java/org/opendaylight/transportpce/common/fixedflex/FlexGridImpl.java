...
public class FlexGridImpl {
    private double start;
    private double stop;
    static final double PRECISION = 10000d;

    public FlexGridImpl(Long index, double centreFrequency, double start, double stop, double wavelength) {
        this.start = start;
        this.stop = stop;
    }
    ...
    public FlexGridImpl getFlexWaveMapping(float centerFrequency, float slotwidth) {
        ...
        flexGridImpl.start = 193.1 + (centerFrequency - slotwidth / 2) / 1000.0;
        flexGridImpl.start = Math.round(flexGridImpl.start * PRECISION) / PRECISION;
        flexGridImpl.stop = 193.1 + (centerFrequency + slotwidth / 2) / 1000.0;
