...
public class NetConfTopologyListener implements DataTreeChangeListener<Node> {
    ...
    private boolean subscribeStream(MountPoint mountPoint, String nodeId) {
        ...
        List<String> listSuppStreams = getSupportedStream(nodeId);
        LOG.info("These are the supported streams {}", listSuppStreams);
        if (listSuppStreams.contains("OPENROADM")) {
            LOG.info("Triggering notification stream OPENROADM for node {}", nodeId);
            ListenableFuture<RpcResult<CreateSubscriptionOutput>> subscription =
                rpcService.createSubscription(
                    new CreateSubscriptionInputBuilder().setStream(new StreamNameType("OPENROADM")).build());

            if (checkSupportedStream("OPENROADM", subscription)) {
                return true;
            }
        } else {
            LOG.info("Triggering notification stream NETCONF, for node {}",  nodeId);
            ListenableFuture<RpcResult<CreateSubscriptionOutput>> subscription =
                rpcService.createSubscription(
                    new CreateSubscriptionInputBuilder().setStream(new StreamNameType("NETCONF")).build());
            if (checkSupportedStream("NETCONF", subscription)) {
                return true;
            }
        }
        return false;
