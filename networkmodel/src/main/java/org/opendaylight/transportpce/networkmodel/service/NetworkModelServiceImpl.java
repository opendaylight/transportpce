...
public class NetworkModelServiceImpl implements NetworkModelService {
        ...
        if (tps.isEmpty()) {
            ...
            for (TerminationPoint tp : tps) {
                if (tp.augmentation(TerminationPoint1.class) != null
                        ...
                        .getTsPool() != null && tp.augmentation(TerminationPoint1.class)
    ...
    private void updateOpenRoadmNetworkTopologyTPs(List<Node> nodesList, String nodeId) {
        ...
        for (Node node : nodesList) {
            ...
            if (abstractNodeId.contains(nodeId) && node.augmentation(org.opendaylight.yang.gen.v1.http.org.openroadm
                    .common.network.rev181130.Node1.class).getOperationalState().equals(State.InService)) {
                ...
                for (TerminationPoint tp : tpList) {
                    ...
                    State tpState = Objects.requireNonNull(tp.augmentation(org.opendaylight.yang.gen.v1.http.org
                            .openroadm.common.network.rev181130.TerminationPoint1.class)).getOperationalState();
                    if (this.terminationPointsChanged.containsKey(tpId) && !this.terminationPointsChanged.get(tpId)
                        ...
                        TerminationPoint updTp = new TerminationPointBuilder().withKey(tp.key()).setTpId(tp.getTpId())
                                .addAugmentation(new TerminationPoint1Builder().setAdministrativeState(newTpAdminState)
                                        .setOperationalState(newTpOperationalState).build()).build();
