...
public class NetworkModelServiceImpl implements NetworkModelService {
    ...
    private List<Link> getSupportingOdu4Links(List<LinkTp> nodesTopoTps, Uint32 serviceRate) {
        ...
        String odtu4OdtuC4 = Uint32.valueOf(100).equals(serviceRate) ? "ODUC4" : "ODTU4";
        if (netw1Opt.isPresent() && netw1Opt.get().getLink() != null) {
            odu4links = netw1Opt
                .get()
                .nonnullLink().values()
                .stream().filter(lk -> lk.getLinkId().getValue().startsWith(odtu4OdtuC4))
        ...
        LOG.debug("odu4oduC4links = {}", links.toString());
        return links;
    }
