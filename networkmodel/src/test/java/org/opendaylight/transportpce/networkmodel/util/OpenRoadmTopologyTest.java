...
public class OpenRoadmTopologyTest {
    private static final Logger LOG = LoggerFactory.getLogger(OpenRoadmTopologyTest.class);
    @Mock
    ...
    public void deleteLinkOkTest() throws InterruptedException, ExecutionException {
        ...
        ListenableFuture<Optional<Link>> linkFuture = new ListenableFuture<Optional<Link>>() {
        ...
        when(networkTransactionService.read(LogicalDatastoreType.CONFIGURATION, linkIID)).thenReturn(linkFuture);
    ...
    public void deleteLinkExceptionTest() {
        String srcNode = "ROADM-A1-DEG1";
        String dstNode = "ROADM-A1-SRG1";
        String srcTp = "DEG1-CTP-TXRX";
        String destTp = "SRG1-CP-TXRX";
...
//            .thenThrow(new InterruptedException());
        doThrow(new InterruptedException()).when(networkTransactionService).commit();
        boolean result = OpenRoadmTopology.deleteLink("ROADM-A1-DEG1", "ROADM-A1-SRG1", "DEG1-CTP-TXRX", "SRG1-CP-TXRX",
            networkTransactionService);
        assertFalse("exception should appear", result);
    }
    ...
    private void checkDegreeNode(String nodeNb, Node node) {
        ...
        assertEquals(Uint16.valueOf(nodeNb), node.augmentation(org.opendaylight.yang.gen.v1.http.org.openroadm.network
            .topology.rev181130.Node1.class).getDegreeAttributes().getDegreeNumber());
        assertEquals(96, node.augmentation(org.opendaylight.yang.gen.v1.http.org.openroadm.network
            .topology.rev181130.Node1.class).getDegreeAttributes().getAvailableWavelengths().size());
    ...
    private void checkSrgNode(String nodeNb, Node node) {
        ...
        assertEquals(96, node.augmentation(org.opendaylight.yang.gen.v1.http.org.openroadm.network
            .topology.rev181130.Node1.class).getSrgAttributes().getAvailableWavelengths().size());
        List<TerminationPoint> tps = node.augmentation(
            org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.network.topology.rev180226.Node1.class)
            .getTerminationPoint().stream()
            .sorted((tp1, tp2) -> tp1.getTpId().getValue().compareTo(tp2.getTpId().getValue()))
