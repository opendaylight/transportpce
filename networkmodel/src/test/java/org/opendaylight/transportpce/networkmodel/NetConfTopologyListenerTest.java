...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.device.rev251028.connection.oper.AvailableCapabilities;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netconf.device.rev251028.credentials.credentials.LoginPwBuilder;
...
public class NetConfTopologyListenerTest {
    ...
    void testOnDataTreeChangedWhenDeleteNode() {
        ...
        when(portMapping.getNode("netconfNode1").getDatamodelType()).thenReturn(NodeDatamodelType
                .valueOf("OPENROADM"));
        when(networkModelService.deleteOpenRoadmnode(anyString())).thenReturn(true);
    ...
    void testOnDataTreeChangedWhenDeleteOpenConfigNode() {
        ...
        when(portMapping.getNode("netconfNode1").getDatamodelType()).thenReturn(NodeDatamodelType
                .valueOf("OPENCONFIG"));
        when(networkModelService.deleteOpenConfignode(anyString())).thenReturn(true);
    ...
    void testOnDataTreeChangedWhenShouldNeverHappen() {
        ...
        final Node netconfNodeBefore = getNetconfNode("netconfNode1",
            ConnectionStatus.Connected, OPENROADM_DEVICE_VERSION_2_2_1);
        when(node.dataBefore()).thenReturn(netconfNodeBefore);
        when(node.dataAfter()).thenReturn(netconfNodeBefore);
