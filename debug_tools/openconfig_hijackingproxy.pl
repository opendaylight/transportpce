...
# messages will be sent in clear text w/o EXI compression so that they can be parsed
    ...
    '<capability>urn:fujitsu:<\/capability>' =>
      's@<capability>urn:fujitsu:[^<]*</capability>@@'
);
...
my %regex_hash2 = (
 ...
 # this removes some proprietary extensions capabilities from server XML payload
    ...
    '\Q"urn:fujitsu:\E'         => 's@="urn:fujitsu:.*@>@g'
);
...
# TODO deal with get-schema - currenlty YANG models retrived can be altered by the code below...
        ## XML payload
        while ( my $nread = sysread( $ssh_subsocket, $buf, 400 ) ) {

# sysread buffer size might split $markin - this completes the buffer until a > is met.
            my $buf2;
            while ( $buf !~ />$/ ) {
                my $nread2 = sysread( $ssh_subsocket, $buf2, 1 );
                $buf .= $buf2;
            }
            my $filtered_out = $buf;
            foreach my $keyword ( keys %regex_hash2 ) {
...
# As proprietary extensions can be used upon any markup, this part of the code will retrieve
# the markup where the proprietary extension was found and remove any attribute to generate 2 regex
# markup patterns: one for the entrance and an other one for the exit of the XML declaration.
# Everything between these 2 patterns will be removed from the messages.
          ...
          # send nothing until markout is found to remove proprietary extensions
                    if ( $filtered_out =~ /$markout_regex/ ) {
                     ...
                     # entrance and exit patterns found in the same buffer block
                        print STDERR "found pattern out " . $markout . "\n";
                    }
                    else {
                 # entrance and exit patterns not found in the same buffer block
                        print STDERR
                          "not found a pattern out - increasing bufffer size";
                        while ( $filtered_out !~ /$markout_regex/ ) {
                            my $nread2 = sysread( $ssh_subsocket, $buf2, 1 );
                            $filtered_out .= $buf2;
                            print STDERR ".";
                        }
                        print STDERR "\nfound pattern out " . $markout . "\n";
                    }
