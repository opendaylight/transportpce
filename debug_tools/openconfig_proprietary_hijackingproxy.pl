...
##           }
##        }
#TODO remove EXI capability in hello message
#TODO use compiled regex hash once ready
#FIXME buf size may split $markout or $mark and generate an infinite loop...
        ...
        $mark .= " ";
        my $markout_regex = qr//;
        eval('$markout_regex= qr/\Q'.$markout2.'\E/;');
        if ($buf =~ /$markout_regex/) {
            print STDERR "found a pattern out\n";
            my $filtered_out = $buf;
            eval('$filtered_out =~ s@\Q'.$mark.'\E.*\Q'.$markout.'\E@@g;');
            ...
            while (my $nread2 = sysread($ssh_subsocket,$buf2,400)) {
                ...
                if ($buf2 =~ /$markout_regex/) {
                    ...
                    eval('$filtered_out =~ s@.*\Q'.$markout.'\E@@g;');
                    print $client $filtered_out;
                    $ssh_subsocket->flush();
                    if (defined($verbose))  { print STDOUT $filtered_out; }
                    last;
