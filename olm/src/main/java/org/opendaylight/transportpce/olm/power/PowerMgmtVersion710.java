...
public final class PowerMgmtVersion710 {
    ...
    public static boolean setTransponderPower(String nodeId, String interfaceName, BigDecimal txPower,
                                              DeviceTransactionManager deviceTransactionManager,
                                              Interface interfaceObj, Mapping portMap) {
        ...
        if ((portMap.getSupportedInterfaceCapability().contains(IfOCHOTU4ODU4.VALUE))
            ...
            OchBuilder ochBuilder = new OchBuilder(ochOtsiInterfaceBuilder.augmentation(org.opendaylight.yang
                    .gen.v1.http.org.openroadm.optical.channel.interfaces.rev200529
                    .Interface1.class).getOch());
            ochBuilder.setTransmitPower(new PowerDBm(Decimal64.valueOf(txPower)));
            ochOtsiInterfaceBuilder.addAugmentation(new org.opendaylight.yang
                    .gen.v1.http.org.openroadm.optical.channel.interfaces.rev200529.Interface1Builder()
                    .setOch(ochBuilder.build()).build());

        } else if (portMap.getSupportedInterfaceCapability().contains(IfOtsiOtsigroup.VALUE)) {

            OtsiBuilder otsiBuilder = new OtsiBuilder(ochOtsiInterfaceBuilder.augmentation(Interface1.class).getOtsi());
            otsiBuilder.setTransmitPower(new PowerDBm(Decimal64.valueOf(txPower)));
            ochOtsiInterfaceBuilder.addAugmentation(new Interface1Builder().setOtsi(otsiBuilder.build()).build());
        }
