...
import org.opendaylight.yang.gen.v1.http.org.openroadm.optical.transport.interfaces.rev200529.Interface1Builder;
import org.opendaylight.yang.gen.v1.http.org.openroadm.optical.transport.interfaces.rev200529.ots.container.Ots;
...
final class OlmUtils710 {
        ...
        try {
            ...
            if (interfaceObject.isPresent()) {
                ...
                if (intf.augmentation(Interface1.class) != null
                    ...
                    Ots ots =
                            intf.augmentation(Interface1.class).getOts();
        ...
        for (Map.Entry<CurrentPmEntryKey, CurrentPmEntry> entry : currentPmEntryList.entrySet()) {
            ...
            for (CurrentPm currentPm:currentPMList) {
                ...
                if (!measurements.isEmpty()) {
                    ...
                    LOG.info("No ParameterValue found for node {} on type {} for granularity {}",
                            input.getNodeId(),
    ...
    private static List<Measurements> getMeasurements(CurrentPm currentPm) {
        ...
        for (Measurement measurement: currentPm.nonnullMeasurement().values()) {
            ...
            if (measurement.getPmParameterValue() != null) {
                pmMeasureBuilder.setPmparameterValue(measurement.getPmParameterValue().stringValue());
        ...
        for (CurrentPm pm : currentPmList) {
            for (Measurement measurements: pm.nonnullMeasurement().values()) {
                if (measurements.getGranularity().getIntValue() == org.opendaylight.yang.gen.v1.http.org.openroadm.pm
                    .types.rev171215.PmGranularity.forValue(input.getGranularity().getIntValue()).getIntValue()) {
                    MeasurementsBuilder pmMeasureBuilder = new MeasurementsBuilder();
