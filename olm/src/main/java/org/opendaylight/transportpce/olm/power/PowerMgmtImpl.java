...
public class PowerMgmtImpl implements PowerMgmt {
        ...
        for (int i = 0; i < input.getNodes().size(); i++) {
            ...
            switch (inputNode.getNodeInfo().getNodeType()) {
                case Xpdr:
                    ...
                    kafkaPublisher.publishNotification("service","Setting Xpdr power: Node " + nodeId + " Interface "
                        + interfaceName + " Power " + powerVal);
                    try {
                        LOG.info("Now going in sleep mode");
                        kafkaPublisher.publishNotification("service","Starting Xpdr " + nodeId + " sleep for "
                            + timer1 + " ms");
                        Thread.sleep(timer1);
                        kafkaPublisher.publishNotification("service","Finished Xpdr " + nodeId + " sleep");
                ...
                case Rdm:
                    ...
                    try {
                        ...
                        kafkaPublisher.publishNotification("service","Setting Rdm power: Node " + nodeId
                            + " Connection " + connectionNumber + " Power " + powerValue);
                        kafkaPublisher.publishNotification("service","Starting Rdm " + nodeId + " sleep for "
                            + timer2 + " ms");
                        Thread.sleep(timer2);
                        kafkaPublisher.publishNotification("service","Finished Rdm " + nodeId + " sleep");
    ...
    public Boolean powerTurnDown(ServicePowerTurndownInput input) {
        ...
        kafkaPublisher.publishNotification("service","OLM Power Turn Down for Node " + input);
        ...
        for (int i = input.getNodes().size() - 1; i >= 0; i--) {
            ...
            try {
                if (destTpId.toUpperCase(Locale.getDefault()).contains("DEG")) {
                    ...
                    kafkaPublisher.publishNotification("service","OLM Power Turn Down for Node "
                            + input + " sleep for " + timer2 + " ms");
                    Thread.sleep(timer2);
                    kafkaPublisher.publishNotification("service","Finished OLM Power Turn Down "
                            + input + " sleep");
