...
public class PowerMgmtImpl implements PowerMgmt {
    ...
    private BigDecimal getRdmPowerValue(BigDecimal spanLossTx, ServicePowerSetupInput input) {
        ...
        // This is Pin[50GHz]
        if ((spanLossTx.doubleValue() >= 6.0) && (spanLossTx.doubleValue()  < 8.0)) {
            powerValue = BigDecimal.valueOf(-3.0);
        } else if ((spanLossTx.doubleValue()  >= 8.0) && (spanLossTx.doubleValue()  < 23.0)) {
            powerValue = BigDecimal.valueOf(- (8.0 - spanLossTx.doubleValue()) / 3.0 - 3.0);
        } else if (spanLossTx.doubleValue()  >= 23.0) {
        ...
        LOG.info("P1[50GHz]={} dBm for spanloss {} based of OR 5.0 power target mask", powerValue, spanLossTx);
        ...
        if (input.getMcWidth() != null) {
            ...
            BigDecimal pinMcWidth = powerValue.add(new BigDecimal(pdsVal, new MathContext(3,
                RoundingMode.HALF_EVEN)));
            LOG.info("P1[{}GHz]={} dB will be used for OSNR calculation", mcWidth, pinMcWidth);
        }
        ...
        LOG.info("The power value {} for spanloss {}", powerValue, spanLossTx);
        return powerValue;
    }
