...
Before creating an OTN service, your topology must contain at least two xpdr devices of type MUXPDR
or SWITCH connected to two different rdm devices. To check that these xpdr are present in the
       ...
       "node-interface": [
         {
           "node-id": "<otn-node-id>",
           "och-interface-id": [
             "XPDR<number>-NETWORK<port-number>-<wavelength-number>"
           ],
           "otu-interface-id": [
             "XPDR<number>-NETWORK<port-number>-OTU4"
           ]
         }
       ]
     }
...
.. code:: json

   {
     "input": {
       ...
       "complex-trib-slots": ["<trib-slot-number-inside-supported-ODU4>"
       ],
       "opucn-trib-slots": [
         "<trib-slot-number-inside-supported-ODU4>"
       ],
