                                    ...
                                    switch (dataType) {
                                        ...
                                        case CIRCUITPACK_CLASS:
                                            ...
                                            if (cpck_aft != null && cpck_bef != null) {
                                                if (operStateChanged(cpck_aft, cpck_bef)) {
                                                    LOG.warn("Some port has changed its operational state");
...
>>>>>>> 282a7fc... updates on docker containers for kafka and mongodb
                                                                .build();
                                                        editList.add(edit);
                                                    }
                                                } else {
                                                    LOG.warn("All ports have the same operational state. No notification emitted");
                                                }
                                            } else {
                                            ...
                                            break;
                                        default:
                                            LOG.info("Type of change = {} not supported", dataType);
                                    }
                                }
                            }
                        }
    ...
    private boolean operStateChanged(CircuitPacks cpacksaft, CircuitPacks cpacksbef) {
        List<Ports> portsList1 = cpacksaft.getPorts();
        List<Ports> portsList2 = cpacksbef.getPorts();
        boolean operStatehaschanged = false;
        for (Ports ports1:portsList1) {
            for (Ports ports2:portsList2) {
                if (ports1.getPortName().equals(ports2.getPortName())) {
                    if (!ports1.getOperationalState().equals(ports2.getOperationalState())) {
                        LOG.info("Change in operational state");
                        operStatehaschanged = true;
                    }
                    break;
                }
            }
            if (operStatehaschanged) {
                break;
            }
        }
        return operStatehaschanged;
    }
