...
tpcetest = TransportPCEtesting()

def setUp(self):
    time.sleep(2)

def connect_xpdrs(self):
    print("Connecting SPDRA")
    response = test_utils.mount_device("SPDR-SA1", ('spdra', self.NODE_VERSION))
    tpcetest.assertEqual(response.status_code, requests.codes.created, test_utils.CODE_SHOULD_BE_201)
    time.sleep(2)
...
def interconnect_roadm_and_xpdr(self):
    ...
    tpcetest.assertIn('Xponder Roadm Link created successfully', response["output"]["result"])
    time.sleep(2)
    response = test_utils.transportpce_api_rpc_request(
        ...
        {'links-input': {'xpdr-node': 'SPDR-SA1', 'xpdr-num': '1', 'network-num': '1',
                         'rdm-node': 'ROADM-A1', 'srg-num': '1', 'termination-point-num': 'SRG1-PP1-TXRX'}})
    tpcetest.assertEqual(response['status_code'], requests.codes.ok)
    tpcetest.assertIn('Roadm Xponder links created successfully', response["output"]["result"])
    time.sleep(2)
    response = test_utils.transportpce_api_rpc_request(
        'transportpce-networkutils', 'init-xpdr-rdm-links',
...
def add_omsAttributes_to_ROADM2ROADM_links(self):
    ...
    tpcetest.assertEqual(response.status_code, requests.codes.created)
...
def cleanup_and_desinstall_tapi_feature(self):
    disconnect_spdrA(self)
    disconnect_spdrC(self)
    disconnect_roadmA(self)
    disconnect_roadmC(self)
    uninstall_Tapi_Feature(self)
...
def disconnect_spdrA(self):
    response = test_utils.unmount_device("SPDR-SA1")
    tpcetest.assertIn(response.status_code, (requests.codes.ok, requests.codes.no_content))

def disconnect_spdrC(self):
    response = test_utils.unmount_device("SPDR-SC1")
    tpcetest.assertIn(response.status_code, (requests.codes.ok, requests.codes.no_content))

def disconnect_roadmA(self):
    response = test_utils.unmount_device("ROADM-A1")
    tpcetest.assertIn(response.status_code, (requests.codes.ok, requests.codes.no_content))

def disconnect_roadmC(self):
    response = test_utils.unmount_device("ROADM-C1")
    tpcetest.assertIn(response.status_code, (requests.codes.ok, requests.codes.no_content))

def uninstall_Tapi_Feature(self):
