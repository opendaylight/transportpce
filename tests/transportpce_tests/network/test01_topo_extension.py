...
class TransportPCEtesting(unittest.TestCase):
    ...
    def test_13_getLinks_OpenroadmTopology(self):
        ...
        for link in response['network'][0]['ietf-network-topology:link']:
            ...
            if (link['transportpce-or-network-augmentation:link-class'] == null) :
                continue
            linkClass = link['transportpce-or-network-augmentation:link-class']
            if (linkType == 'ROADM-ROADM' & linkClass == 'inter-domain') :
                find = linkId in check_list[linkType]
                self.assertEqual(find, True)
                interDomainLinkNber +=1
            if (linkType == 'XPONDER-OUTPUT' & linkClass == 'alien-to-tapi') :
                find = linkId in check_list[linkType]
                self.assertEqual(find, True)
                alienToTapiLinkNber +=1
            if (linkType == 'XPONDER-INPUT' & linkClass == 'alien-to-tapi') :
                find = linkId in check_list[linkType]
                self.assertEqual(find, True)
                alienToTapiLinkNber +=1
