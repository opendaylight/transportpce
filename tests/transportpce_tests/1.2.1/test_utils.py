...
def start_roadma_full_honeynode():
    if os.path.isfile(honeynode_executable):
        with open('honeynode2.log', 'w') as outfile:
            return subprocess.Popen(
                [honeynode_executable, "17831", os.path.join(samples_directory,"oper-ROADMA-full.xml")],
                stdout=outfile, stderr=outfile)
...
def start_roadmc_full_honeynode():
    if os.path.isfile(honeynode_executable):
        with open('honeynode3.log', 'w') as outfile:
            return subprocess.Popen(
                [honeynode_executable, "17833", os.path.join(samples_directory,"oper-ROADMC-full.xml")],
                stdout=outfile, stderr=outfile)
