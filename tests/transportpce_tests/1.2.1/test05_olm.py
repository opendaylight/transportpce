...
class TransportOlmTesting(unittest.TestCase):
    ...
    def test_15_get_OTS_DEG1_TTP_TXRX_ROADMA(self):
        ...
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertIn(
            response['org-openroadm-optical-transport-interfaces:ots']['span-loss-transmit'],
            ('5.7', 5.7))
        self.assertIn(
            response['org-openroadm-optical-transport-interfaces:ots']['span-loss-receive'],
            ('15.1', 15.1))
        # FIXME test_utils_rfc8040.check_node_attribute2_request retrives caluesin config DS
    ...
    def test_20_get_interface_XPDRA_XPDR1_NETWORK1(self):
        ...
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertIn(
            response['org-openroadm-optical-channel-interfaces:och']['transmit-power'],
            ('0', 0))
        self.assertIn(
            response['org-openroadm-optical-channel-interfaces:och']['wavelength-number'],
            ('1', 1))
    ...
    def test_21_get_roadmconnection_ROADMA(self):
        ...
        self.assertIn(response['roadm-connections'][0]['target-output-power'], ('-3.30', -3.3))
    ...
    def test_25_get_roadmconnection_ROADMC(self):
        ...
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertEqual("gainLoss", response['roadm-connections'][0]['opticalControlMode'])
        self.assertIn(response['roadm-connections'][0]['target-output-power'], ('-0.63', -0.63))
    ...
    def test_27_get_roadmconnection_ROADMA(self):
        ...
        self.assertIn(response['roadm-connections'][0]['target-output-power'], ('-60', -60))
