...
class TransportTapitesting(unittest.TestCase):
    ...
    def test_01_get_tapi_topology_T100G(self):
        self.tapi_topo["tapi-topology:input"]["tapi-topology:topology-id-or-name"] = test_utils_rfc8040.T100GE
    ...
    def test_02_get_tapi_topology_T0(self):
        self.tapi_topo["tapi-topology:input"]["tapi-topology:topology-id-or-name"] =\
            test_utils_rfc8040.T0_MULTILAYER_TOPO
        response = test_utils_rfc8040.tapi_rpc_request(
            'tapi-topology', 'get-topology-details', self.tapi_topo)
        self.assertEqual(response['status_code'], requests.codes.ok)
    ...
    def test_04_check_tapi_topos(self):
        ...
        self.tapi_topo["input"]["topology-id-or-name"] = test_utils_rfc8040.T0_MULTILAYER_TOPO
        response = test_utils_rfc8040.tapi_rpc_request(
            'tapi-topology', 'get-topology-details', self.tapi_topo)
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertEqual(len(response["output"]["topology"]["node"]), 1, 'Topology should contain 1 node')
