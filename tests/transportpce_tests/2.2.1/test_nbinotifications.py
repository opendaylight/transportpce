...
class TransportNbiNotificationstesting(unittest.TestCase):
    ...
    def test_17_change_status_port_roadma_srg(self):
    ...
    def test_18_check_update_service1(self):
        response = test_utils.get_service_list_request("services/service1")
        self.assertEqual(response.status_code, requests.codes.ok)
        res = response.json()
    ...
    def test_19_get_notifications_service1(self):
        data = {
            "input": {
                "connection-type": "service",
                "id-consumer": "consumer",
        ...
        self.assertEqual(res['output']['notification-service'][-1]['service-name'], 'service1')
        self.assertEqual(res['output']['notification-service'][-1]['connection-type'], 'service')
        self.assertEqual(res['output']['notification-service'][-1]['operational-state'], 'outOfService')
        self.assertEqual(res['output']['notification-service'][-1]['message'], 'The service is now Out of Service')
        time.sleep(2)

    def test_20_delete_eth_service1(self):
    ...
    def test_21_get_notifications_service1(self):
        data = {
            "input": {
                "connection-type": "service",
                "id-consumer": "consumer",
    ...
    def test_22_disconnect_XPDRA(self):
        response = test_utils.unmount_device("XPDR-A1")
        self.assertEqual(response.status_code, requests.codes.ok, test_utils.CODE_SHOULD_BE_200)

    def test_23_disconnect_XPDRC(self):
        response = test_utils.unmount_device("XPDR-C1")
        self.assertEqual(response.status_code, requests.codes.ok, test_utils.CODE_SHOULD_BE_200)

    def test_24_disconnect_ROADMA(self):
        response = test_utils.unmount_device("ROADM-A1")
        self.assertEqual(response.status_code, requests.codes.ok, test_utils.CODE_SHOULD_BE_200)

    def test_25_disconnect_ROADMC(self):
