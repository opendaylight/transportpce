...
class TransportPCEPortMappingTesting(unittest.TestCase):
    ...
    def test_03_rdm_portmapping_info(self):
        response = test_utils_rfc8040.get_portmapping_node_node_info("ROADM-D1")
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertEqual(
            {'node-type': 'rdm',
    ...
    def test_04_rdm_deg1_lcp(self):
        ...
        response = test_utils_rfc8040.get_portmapping_node_mc_capa("ROADM-D1", "DEG1-TTP")
        self.assertEqual(response['status_code'], requests.codes.ok)
    ...
    def test_05_rdm_deg2_lcp(self):
        # pylint: disable=line-too-long
        response = test_utils_rfc8040.get_portmapping_node_mc_capa("ROADM-D1", "DEG2-TTP")
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertIn(response['mc-capabilities'],
    ...
    def test_06_rdm_srg1_lcp(self):
        # pylint: disable=line-too-long
        response = test_utils_rfc8040.get_portmapping_node_mc_capa("ROADM-D1", "SRG1-PP")
        self.assertEqual(response['status_code'], requests.codes.ok)
        self.assertIn(response['mc-capabilities'],
