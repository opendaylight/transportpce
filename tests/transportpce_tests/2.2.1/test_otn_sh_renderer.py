...
class TransportPCEtesting(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.processes = test_utils.start_sims(['spdrav2','spdrcv2'])
    ...
    def test_02_connect_SPDR_SC1(self):
        response = test_utils.mount_device("SPDR-SC1", 'spdrcv2')
        self.assertEqual(response.status_code, requests.codes.created,
                         test_utils.CODE_SHOULD_BE_201)
        time.sleep(10)
    ...
    def test_03_service_create_OTU4(self):
        url = "{}/operations/transportpce-renderer:service-" \
              "implementation-request".format(test_utils.RESTCONF_BASE_URL)
        data = {
    ...
    def test_08_service_create_OTU4(self):
        ...
        response = test_utils.post_request(url, data, test_utils.ODL_LOGIN,
                                           test_utils.ODL_PWD)
        time.sleep(3)
        self.assertEqual(response.status_code, requests.codes.ok)
        res = response.json()
