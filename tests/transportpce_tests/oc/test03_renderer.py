...
class TransportpceOCRendererTesting(unittest.TestCase):
    ...
    def test_14_service_path_create_client_check_properties(self):
        for optchannelid in TransportpceOCRendererTesting.optical_channel_id:
            ...
            self.assertEqual(response['status_code'], requests.codes.ok)
            for property in response['openconfig-platform:properties']['property']:
                self.assertEqual(property['name'], 'tx-dis')
                self.assertEqual(property['config'], {'name': 'tx-dis', 'value': 'FALSE'})
                self.assertEqual(property['state'], {'name': 'tx-dis', 'value': 'FALSE', 'configurable': True})
    ...
    def test_18_service_path_delete_client_check_properties(self):
        ...
        for property in response['openconfig-platform:properties']['property']:
            self.assertEqual(property['name'], 'tx-dis')
            self.assertEqual(property['config'], {'name': 'tx-dis', 'value': 'TRUE'})
            self.assertEqual(property['state'], {'name': 'tx-dis', 'value': 'TRUE', 'configurable': True})
