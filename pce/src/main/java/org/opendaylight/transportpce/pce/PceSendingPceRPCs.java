...
public class PceSendingPceRPCs {
    ...
    private void pathComputationWithConstraints(PceConstraints hardConstraints, PceConstraints softConstraints,
        ...
        PceGraph graph = new PceGraph(nwAnalizer.getaendPceNode(), nwAnalizer.getzendPceNode(),
                ...
                rc, serviceType, networkTransaction, mode, getBitMapConstraint(input.getCustomerName()));
        graph.calcPath();
    ...
    private BitSet getBitMapConstraint(String customerName) {
        BitSet referenceBitSet = new BitSet(GridConstant.EFFECTIVE_BITS);
        referenceBitSet.set(1, GridConstant.EFFECTIVE_BITS, true);
        ...
        try {
            if (networkTransaction.read(LogicalDatastoreType.CONFIGURATION, sfIID).get().isPresent()) {
                ...
                for (Map.Entry<SpectrumFillingRulesKey, SpectrumFillingRules> rule:
                    ...
                    if (customerName != null && rule.getValue().getSpectrumRangeOfAppliance().getNonAuthorizedCustomer()
                            .contains(customerName)) {
                        //Customer shall not be put in this spectrum portion
                        referenceBitSet.set(
                            GridUtils.getIndexFromFrequency(startFreq.getValue()),
                            GridUtils.getIndexFromFrequency(stopFreq.getValue()) + 1, false);
                    ...
                    } else if (customerName != null && rule.getValue().getSpectrumRangeOfAppliance()
                            .getDedicatedCustomer().contains(customerName)) {
                        // Spectrum portion is dedicated to customers including this one
                        referenceBitSet.set(
                            GridUtils.getIndexFromFrequency(startFreq.getValue()),
                            GridUtils.getIndexFromFrequency(stopFreq.getValue()) + 1, true);
                    ...
                    } else if (!rule.getValue().getSpectrumRangeOfAppliance().getDedicatedCustomer()
                            .isEmpty()) {
