...
public class ServiceCreateClientInput implements ClientInput {
    ...
    public @NonNull Collection clientRangeWishListIntersection() {
        return intervalFactory.frequencyWindow(pathComputationRequestInput, spectrum);
    }

    @Override
    public @NonNull Collection clientRangeWishListSubset() {
        return intervalFactory.flexGrid(pathComputationRequestInput, spectrum);
    }

    @Override
    public int slots(int spectralWidth) {
