...
public class OtnDeviceRendererServiceImpl implements OtnDeviceRendererService {
            ...
            case "Ethernet":
                if ("10G".equals(input.getServiceRate()) || "1G".equals(input.getServiceRate())) {
                    try {
                        ...
                        Map<String, String> nodesToUpdate = new HashMap<>();
                        if (!nodeInterfaces.isEmpty()) {
                            for (NodeInterface nodeInterf : nodeInterfaces) {
                                if (nodeInterf.getOduInterfaceId() != null) {
                                    Optional<String> interOpt = nodeInterf.getOduInterfaceId().stream()
                                        .filter(id -> id.contains("NETWORK")).findFirst();
                                    if (interOpt.isPresent()) {
                                        String tp = interOpt.get().split("-ODU")[0];
                                        nodesToUpdate.put(nodeInterf.getNodeId(), tp);
                                    }
                                }
                            }
                        }
                    ...
                    LOG.warn("Invalid serivce-rate for service-type Ethernet");
                }
                break;
            case "ODU":
                if ("100G".equals(input.getServiceRate())) {
                    ...
                    LOG.warn("Invalid serivce-rate for service-type ODU");
                }
                break;
            default:
    ...
    public OtnServicePathOutput deleteOtnServicePath(OtnServicePathInput input) {
        ...
        Map<String, String> nodeTpToUpdate = new HashMap<>();
        CopyOnWriteArrayList<Nodes> otnNodesProvisioned = new CopyOnWriteArrayList<>();
        ...
        ForkJoinTask forkJoinTask = forkJoinPool.submit(() -> nodes.parallelStream().forEach(node -> {
            ...
            if (this.deviceTransactionManager.isDeviceMounted(nodeId)) {
                ...
                switch (input.getServiceRate()) {
                    case ("100G"):
                        ...
                        break;
                    case ("10G"):
                        connectionNumber = clientTp + ODU2E + input.getServiceName() + "-x-" + networkTp
                            + ODU2E + input.getServiceName();
                        break;
                    case ("1G"):
                        connectionNumber = clientTp + "-ODU0-" + input.getServiceName() + "-x-" + networkTp
                            + "-ODU0-" + input.getServiceName();
                        break;
                    default:
            ...
            Optional<String> interfOpt = interfacesToDelete.stream().filter(ele -> ele.contains("NETWORK")).findFirst();
            if (interfOpt.isPresent()) {
                nodeTpToUpdate.put(nodeId, interfOpt.get().split("-ODU")[0]);
            }
        }));
        try {
            forkJoinTask.get();
        ...
        if (!nodeTpToUpdate.isEmpty() && !"ODU".equals(input.getServiceType())) {
            updateOtnTopology(null, nodeTpToUpdate, input.getServiceRate(), input.getTribPortNumber(),
    ...
    private void updateOtnTopology(CopyOnWriteArrayList<Nodes> nodes, Map<String, String> nodesTps, String serviceRate,
        Short tribPortNb, Short tribSoltNb, boolean isDeletion) {
        if (nodes != null && nodes.size() == 2) {
            ...
            this.networkModelService.updateOtnLinks(nodesTps, serviceRate, tribPortNb, tribSoltNb, isDeletion);
        }
    }
