                ...
                if (intToDelete != null) {
                    for (String interf : intToDelete) {
                        if (!this.openRoadmInterfaceFactory.isUsedByOtnXc(nodeId, interf, connectionNumber,
                            ...
                            String supportedInterface = this.openRoadmInterfaces.getSupportedInterface(nodeId,
                                    interf);
                            if (!input.getServiceRate().equals("100G") && !supportedInterface.contains("ODU4")) {
                                interfacesToDelete.add(supportedInterface);
                            }
                            if (input.getServiceRate().equals("100G") && !supportedInterface.contains("ODUC4")) {
                                interfacesToDelete.add(supportedInterface);
                            }
                        }
                    }
