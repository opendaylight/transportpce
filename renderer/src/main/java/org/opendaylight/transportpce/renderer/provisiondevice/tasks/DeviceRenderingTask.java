...
public class DeviceRenderingTask implements Callable<DeviceRenderingResult> {
    ...
    public DeviceRenderingResult call() throws Exception {
        ...
        if (!output.getSuccess()) {
            ...
            //calling kafka publisher from DeviceRendererServiceImpl
            if (deviceRenderer instanceof DeviceRendererServiceImpl) {
                ((DeviceRendererServiceImpl) deviceRenderer).getKafkaPublisher().publishNotification("service", "Device rendering failed!");
            }
        ...
        //calling kafka publisher from DeviceRendererServiceImpl
        if (deviceRenderer instanceof DeviceRendererServiceImpl) {
            ((DeviceRendererServiceImpl) deviceRenderer).getKafkaPublisher().publishNotification("service", "Device rendering finished successfully.");
        }
